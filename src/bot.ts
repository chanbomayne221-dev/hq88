  ...
  bot.onText(/\/nap(?:\s+(\d+))?/, async (msg, match) => {
    try {
      ...
      if (lastDep) {
        ...
        if (diffSec < cooldown) {
          const wait = cooldown - diffSec;
          await bot.sendMessage(chatId, `⏳ Bạn vừa tạo lệnh nạp gần đây. Vui lòng chờ *${wait}s* nữa rồi tạo lệnh tiếp theo.`, { parse_mode: "Markdown" });
          return;
        }
      }
      const row = db.prepare(`INSERT INTO pending_deposits (user_id, telegram_id, group_chat_id, amount) VALUES (?, ?, ?, ?)`).run(user.id, telegramId, chatId, amount) as any;
