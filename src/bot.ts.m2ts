  ...
  bot.onText(/\/nap(?:\s+(\d+))?/, async (msg, match) => {
    try {
      ...
      if (lastDep) {
        ...
        const diffSec = Math.floor((Date.now() - lastTime) / 1000);
        const cooldown = 90; // 1 phút 30 giây
        if (diffSec < cooldown) {
          const wait = cooldown - diffSec;
          const m = Math.floor(wait / 60);
          const s = wait % 60;
          const waitStr = m > 0 ? `${m}p${s.toString().padStart(2, "0")}s` : `${s}s`;
          await bot.sendMessage(chatId, `⏳ Bạn vừa tạo lệnh nạp gần đây. Vui lòng chờ *${waitStr}* nữa rồi tạo lệnh tiếp theo.`, { parse_mode: "Markdown" });
          return;
        }
      }
      // Nội dung chuyển khoản ngẫu nhiên để dễ đối soát
      ...
      const BANK_BIN = "970449"; // LPBANK
      const BANK_ACC = "0344127655";
      const ACC_NAME = "VU BAN SUP";
      const qrUrl =
        `https://api.vietqr.io/image/${BANK_BIN}-${BANK_ACC}-HxJXGbk.jpg` +
        `?accountName=${encodeURIComponent(ACC_NAME)}` +
        `&amount=${amount}` +
      ...
      const caption =
        ...
        `⏳ Hiệu lực: ~10 phút\n\n` +
        `⚠️ Vui lòng chuyển *đúng số tiền* và *đúng nội dung* để được duyệt tự động.`;
      try {
        await bot.sendPhoto(chatId, qrUrl, { caption, parse_mode: "Markdown" });
      } catch (e: any) {
